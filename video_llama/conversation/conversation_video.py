...
class Chat:
    ...
    def upload_video(self, video_path, conv, img_list, n_frms):
    ...
    def upload_video_without_audio(self, video_path, conv, img_list, n_frms):
        msg = ""
        if isinstance(video_path, str):  # is a video path
            ext = os.path.splitext(video_path)[-1].lower()
            print(video_path)
    ...
    def upload_img(self, image, conv, img_list):
        ...
        if isinstance(image, str):  # is a image path
            raw_image = Image.open(image).convert('RGB') # 增加一个时间维�?            image = self.image_vis_processor(raw_image).unsqueeze(0).unsqueeze(2).to(self.device)
